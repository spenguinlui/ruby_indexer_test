...
三個目標都是真實生產 Rails app(非玩具範例),以 `scc 3.7.0` 量測(已套用 .gitignore)。每個 case 在對應 app 的**整個 codebase** 裡找 —— 這就是搜尋空間,也是各題的對比依據。

| codebase | Ruby(檔 / 行) | YAML(檔 / 行) | ERB(檔 / 行) | 全部(檔 / 行) | 大小 | 對應 cases |
|---|---|---|---|---|---|---|
| **App-A** | 2,763 / 144.7k | 132 / 13.1k | 34 / 4.1k | 4,134 / 401k | 24 MB | 1,2,3,7,8,9,10,12,13(9)|
| **App-B** | 1,438 / 62.3k | 154 / 6.2k | 20 / 1.3k | 2,657 / 293k | 20 MB | 5,6,11(3)|
| **App-C** | 1,315 / 108.2k | 54 / 3.5k | 17 / 0.8k | 2,507 / 373k | 23 MB | 4,4A(2)|

> rubydex 只索引 Ruby(.rb);**YAML / ERB 是它的盲點** —— case-5(worker 排程在 YAML)、case-10(i18n key 在 YAML)正是踩這個。三個 app 的 Ruby 量在 6 萬–14.5 萬行,grep 的搜尋空間(總程式碼行)達 29 萬–40 萬行。下方詳解每題會標出受測 App。
...
- **小模型贏大模型**:論 recall,**Haiku 4.5 全面勝過 Opus 4.7** —— Haiku-grep −1,贏過 Opus-grep −3 與兩個 Opus+rubydex 條件(−27 / −29);連「完全對」也 Haiku 8 ≥ Opus 7–8。便宜的小模型在「找全」上反而更穩,Opus(low effort)的少探索 + case-1 鎖死反咬一口。
